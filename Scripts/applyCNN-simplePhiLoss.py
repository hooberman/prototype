...
radiograph.
    ...
    python applyCNN-simplePhiLoss.py Run152_list_formatted_1000photons_innerRing3.txt

        -> Run152_list_formatted_1000photons_innerRing3_CNN.txt
           Run152_list_formatted_1000photons_innerRing3_CNN.png  (+ .pdf)
...
BOTH head layouts are supported, and which one a model has is detected from
the width of its output, not from the config:

    3 outputs = (theta_norm, s, c)      the sin/cos head.  phi = atan2(s, c),
                                        and |(s, c)| is a free per-event
                                        confidence in phi.
    2 outputs = (theta_norm, phi_rad)   the simple-phi head, trained with a
                                        wrapped dphi^2 loss.  phi comes back
                                        directly in radians and there is NO
                                        per-event confidence -- the radial
                                        degree of freedom that provided it
                                        was deliberately removed.

theta is the same either way: theta_norm * theta_std + theta_mean.

Everything that used the confidence degrades cleanly on a 2-output model:
--min-conf is refused with a clear message, the scatter radiograph draws
plain dots instead of colour-by-confidence, the phi_conf column in the text
output is written as zeros with a header line saying so, and the confidence
...
def predict(model, cfg, images, batch_size=256):
    """-> (theta_deg, phi_deg, conf, has_conf).

    The head layout is read off the width of the network's output rather
    than trusted from the config, so a model directory with a stale
    phi_formula still decodes correctly:

        3 outputs  (theta_norm, s, c)      phi = atan2(s, c)
                                           conf = |(s, c)|,  has_conf True
        2 outputs  (theta_norm, phi_rad)   phi = degrees(out[1])
                                           conf = zeros,     has_conf False

    When has_conf is False the conf array is still returned, as zeros, so
    every caller keeps the same shape -- but nothing should interpret it.
    """
    import torch
    ...
    if nout == 3:
        ...
        conf, has_conf = np.hypot(s, c), True
    elif nout == 2:
        phi = np.degrees(o[:, 1])
        conf, has_conf = np.zeros_like(phi), False
    else:
        sys.exit("this model has %d outputs; expected 3 (theta_norm, s, c) "
                 "or 2 (theta_norm, phi_rad)" % nout)

    phi = (phi - post["phi_lo"]) % 360.0 + post["phi_lo"]
    return theta, phi, conf, has_conf
...
def write_event_text(img, out_txt, trgid, theta, phi, conf):
    ...
    the file parses with a bare loadtxt/split.  On a 2-output model the
    fourth number is always 0 -- the field is kept so the format does not
    change between models, not because it means anything there.
    """
...
def make_page(theta, phi, conf, out_paths, title, subtitle,
    ...
    has_conf=False (a 2-output model) drops the confidence colouring from the
    scatter radiograph -- colouring every point by a column of zeros would
    just be a lie with a colourbar attached.
    """
    n = theta.size
    ...
    if use_scatter and show_target:
        ...
        # 2-output model: there is no per-event confidence to encode
        ax_r.scatter(np.radians(phi), theta, s=14, color=DOT_COLOR,
                     alpha=0.85, edgecolors="none", zorder=3)
        ax_cb.set_visible(False)
...
def main(argv=None):
    ...
    p.add_argument("--min-conf", type=float, default=None,
                   help="drop events whose phi confidence |(s,c)| is below "
                        ...
                        "3-output sin/cos model; refused on a 2-output one")
    ...
    theta, phi, conf, has_conf = predict(model, cfg, images, args.batch_size)
    print("  head     %s"
          % ("3 outputs (theta_norm, s, c): phi = atan2(s, c), per-event "
             "confidence |(s,c)| available" if has_conf else
             "2 outputs (theta_norm, phi_rad): phi read directly, NO "
             "per-event confidence"))
    print("-" * 78)

    if args.min_conf is not None and not has_conf:
        sys.exit("--min-conf needs a 3-output sin/cos model.  This one has a "
                 "single phi output and no per-event confidence, so there is "
                 "nothing to cut on.")
    ...
    with open(out_txt, "w") as f:
        ...
        f.write("#   trained %s from %s\n"
                % (prov.get("trained_at", "?"),
                   os.path.basename(prov.get("training_file", "?"))))
        if has_conf:
            f.write("# theta, phi in degrees.  phi_conf = |(s,c)|, the "
                    "network's confidence in phi (1 = sure, 0 = azimuthally "
                    "ambiguous).\n")
        else:
        ...
        for i in idx[:cap]:
            ...
            if args.showEventDisplays:
                ...
                sub = ("CNN:  $\\theta$ = %.1f$^\\circ$,  $\\phi$ = "
                       ...
                       % (theta[i], phi[i], conf_bit, int(images[i].sum()),
                          NROW * NRING,
