...
def main(argv=None):
    ...
    p.add_argument("--keep-zeros", action="store_true",
                   help="also histogram events whose total is exactly 0 "
                        "(default: drop them; they are counted and reported "
                        "either way)")
    ...
    # ---- how much of the spike at the left edge is exactly zero ----
    all_totals = totals
    nzero = int(np.sum(all_totals == 0))
    if args.keep_zeros:
        nzero_cut = 0
    else:
        totals = all_totals[all_totals > 0]
        nzero_cut = nzero
        if totals.size == 0:
            sys.exit("every event has a total of exactly 0 -- nothing to plot "
                     "(use --keep-zeros to see them)")
    ...
    print("-" * 78)
    # is the spike at the left edge exactly zero, or just small?
    binw = (hi - lo) / args.bins
    nfirst = int(np.sum((all_totals >= lo) & (all_totals < lo + binw)))
    print("zeros       %d of %d events have a total of exactly 0 (%.1f %%)"
          % (nzero, nev, 100.0 * nzero / nev))
    print("            first bin (%g-%g) holds %d events, of which %d are "
          "exactly 0 (%.1f %%)"
          % (lo, lo + binw, nfirst, nzero,
             100.0 * nzero / nfirst if nfirst else 0.0))
    sm = all_totals[(all_totals > 0) & (all_totals >= lo) &
                    (all_totals < lo + binw)]
    if sm.size:
        print("            the %d non-zero events in that bin run %.0f to %.0f"
              % (sm.size, sm.min(), sm.max()))
    print("            %s"
          % ("zeros DROPPED from the histogram (--keep-zeros to include them)"
             if nzero_cut else "zeros kept in the histogram"))
    print("-" * 78)
    print("total photons per event over the %d summed SiPMs%s:"
          % (len(summed), ", zeros excluded" if nzero_cut else ""))
    ...
    sub = ("%d events read%s  |  sum over %d SiPMs (all except columns 0-2 "
           "of A-D, C4 and C12)\n%s"
           % (nev, ", %d with total 0 excluded" % nzero_cut if nzero_cut
              else "", len(summed), os.path.basename(args.textfile)))

    make_plot(totals, paths, "Total photons per event", sub,
              lo, hi, args.bins, logy=args.logy, nzero=nzero_cut)
    for path in paths:
        print("wrote %s" % path)
